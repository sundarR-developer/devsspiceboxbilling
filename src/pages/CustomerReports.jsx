...
const CustomerReports = () => {
  ...
  const downloadReport = async (period) => {
    setLoading(true);
    try {
      const response = await api.get(`/reports/customers?period=${period}`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `customer_report_${period}.xlsx`);
  ...
  return (
    <Layout title="Customer Reports">
      <div className="card" style={{ background: '#1a1a1a' }}>
        <h3 style={{ color: '#ff9800' }}>Customer Activity</h3>
        <p style={{ color: '#ddd' }}>Download Excel report of customers who ordered in the selected period, showing number of orders and total spent.</p>
